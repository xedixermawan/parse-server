...
class MongoStorageAdapter {
  ...
  createClass(className, schema) {
    ...
    return this.setIndexesWithSchemaFormat(className, schema.indexes, {}, schema.fields).then(() => this._schemaCollection()).then(schemaCollection => schemaCollection.insertSchema(mongoObject));
  }
  ...
  distinct(className, schema, query, fieldName) {
    ...
    return this._adaptiveCollection(className).then(collection => collection.distinct(fieldName, (0, _MongoTransform.transformWhere)(className, query, schema))).then(objects => objects.map(object => {
      if (isPointerField) {
  ...
  aggregate(className, schema, pipeline, readPreference) {
    ...
    pipeline = pipeline.map(stage => {
      ...
      if (stage.$match) {
        const matchTransform = (0, _MongoTransform.transformWhere)(className, stage.$match, schema);
